...
- Gnosis variant: with a gnosis daemon running (`./gradlew :app:run -Pnetwork=gnosis`), once `-Pnetwork=gnosis -Pargs=beacon-status` returns "state":"SYNCED", `-Pnetwork=gnosis -Pargs="get-account <addr>"` should return "verifyMethod":"headerChain". Refresh the trust anchor first with `./gradlew refreshCheckpoint -Pnetwork=gnosis` (one task for every network, writing both engines from one fetch; it refuses to write a root fewer than two providers agree on — see docs/multichain-design.md).
