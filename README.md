...
- **Index freshness depends on the publisher.** The manifest CID now tracks the on-chain publication, so the index is only as fresh as the publisher's latest publish — this can still lag the head substantially (much fresher than the old hardcoded CID, but not real-time). The `latestIndexedBlock`/`headBlock` fields (and the desktop caption) show the actual gap; transactions above `latestIndexedBlock` will not be found.
