...
/// stay well under 1 KiB, and geth refuses a first handshake message (Hello
/// or Disconnect) over 2 KiB (`baseProtocolMaxMsgSize`). An owned decode
/// costs about 50 heap bytes per input byte, so a frame at the full
/// [`MAX_FRAME_BODY_SIZE`] costs about 0.5 GB (#454); this cap keeps it under
/// 1 MB. Decoders refuse a larger message instead of building it.
