...
1–20 s when a peer is slow. Block, receipt and state reads now ask a second
peer after 3 s without an answer, which trims most of that tail; the
optimistic tail's own head-edge fetch is not hedged yet. Myotis's own log (`Logs` tab, filter `access`) shows every request
