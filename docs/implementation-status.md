...
**Not implemented:** discv5 for CL peer discovery. CL peers are seeded from three sources (in that order): the persistent `CLPeerCache`, hardcoded multiaddrs in `NetworkConfig.clPeerMultiaddrs()`, and EIP-1459 DNS resolution of `clEnrTreeUrls`. The DNS pipeline runs on startup in parallel with EL resolution and will emit CL peers as soon as a tree URL is pinned, but `clEnrTreeUrls` is currently empty on mainnet — no canonical CL tree exists yet (each CL client team publishes their own), so today only the cache and hardcoded multiaddrs contribute CL peers. (EL peer DNS discovery is separate and fully populated — see Section 4.)
